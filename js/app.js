(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        document.querySelectorAll("[data-hover-effect]");
        ...
        class MousePRLX {
            constructor(props, data = null) {
                ...
                if (this.config.init) {
                    ...
                    } else this.setLogging("Нет ни одного объекта. Сплю...");
                }
            }
        ...
        let formValidate = {
            getErrors(form) {
                ...
                if (formRequiredItems.length) {
                    ...
                    const passwordInputs = Array.from(form.querySelectorAll("[data-required-password]"));
                    if (passwordInputs.length > 1) {
                        const passwordsMatch = passwordInputs.every((input => input.value === passwordInputs[0].value && input.value.length >= 8));
                        if (!passwordsMatch) {
                            passwordInputs.forEach((input => this.addError(input)));
                            error += passwordInputs.length;
                        } else passwordInputs.forEach((input => this.removeError(input)));
                    } else if (passwordInputs.length === 1) {
                        const passwordInput = passwordInputs[0];
                        if (!passwordInput.value.trim() || passwordInput.value.length < 8) {
                            this.addError(passwordInput);
                            error++;
                        } else this.removeError(passwordInput);
                    }
                }
                return error;
            },
            validateInput(formRequiredItem) {
                let error = 0;
                if (formRequiredItem.type === "password") if (!formRequiredItem.value.trim() || formRequiredItem.value.length < 8) {
                    this.addError(formRequiredItem);
                    error++;
                } else this.removeError(formRequiredItem); else if (formRequiredItem.type === "tel") {
                    const phoneNumber = formRequiredItem.value.replace(/\D/g, "");
        ...
        function formSubmit() {
            ...
            function handleFormError() {
                document.querySelector(".popup__content")?.classList.remove("_sending");
            }
        ...
        class ScrollWatcher {
            ...
            getScrollWatcherConfig(paramsWatch) {
                ...
                if (paramsWatch.margin.indexOf("px") < 0 && paramsWatch.margin.indexOf("%") < 0) {
                    this.scrollWatcherLogging(`ой, настройки data-watch-margin нужно задавать в PX или %`);
                    return;
                }
        ...
        function windowLoad() {
            ...
            const themeButton = document.querySelector(".dark-mode__inner");
            ...
            document.addEventListener("click", (e => {
                if (e.target.closest(".dark-mode__inner")) {
        ...
        const blocksConfig = {
            block1: {
                containerSelector: ".products__items",
                buttonSelector: ".products__button-more ",
                initialDisplayCount: 3,
                loadMoreCount: 3
            }
        };
        document.addEventListener("DOMContentLoaded", (function() {
            for (const blockKey in blocksConfig) if (blocksConfig.hasOwnProperty(blockKey)) {
                const blockConfig = blocksConfig[blockKey];
                startItem[blockKey] = 0;
                endItem[blockKey] = blockConfig.initialDisplayCount;
                initBlock(blockKey, blockConfig);
            }
        }));
        function buildItem(container, item, blockKey) {
            let itemTemplate = "";
            if (blockKey === "block1") itemTemplate = buildTrailersItem(item);
            container.insertAdjacentHTML("beforeend", itemTemplate);
            initRating(container.lastElementChild.querySelector(".rating"));
        }
        function initRating(rating) {
            let ratingActive, ratingValue;
            initRatingVars(rating);
        ...
        function buildTrailersItem(item) {
            let trailersItemTemplate = ``;
            trailersItemTemplate += `<article data-id="${item.id}" class="products__item item-products">`;
            if (item.image) trailersItemTemplate += `<a href="#" class="item-products__image">\n        <img src="${item.image}" alt="Product">\n      </a>`;
            if (item.title && item.starRatingValue && item.starRatingQuantity) trailersItemTemplate += `\n  <div class="item-products__content content-item-products">\n\t\t\t\t\t\t\t\t\t<a href="#" class="content-item-products__title">\n                    ${item.title}\n\t\t\t\t\t\t\t\t\t</a>\n\t\t\t\t\t\t\t\t\t<div class="content-item-products__rating rating rating_set">\n\t\t\t\t\t\t\t\t\t\t<div class="rating__body">\n\t\t\t\t\t\t\t\t\t\t\t<div class="rating__active"></div>\n\t\t\t\t\t\t\t\t\t\t\t<div class="rating__items">\n\t\t\t\t\t\t\t\t\t\t\t\t<input type="radio" class="rating__item" value="1" name="rating">\n\t\t\t\t\t\t\t\t\t\t\t\t<input type="radio" class="rating__item" value="2" name="rating">\n\t\t\t\t\t\t\t\t\t\t\t\t<input type="radio" class="rating__item" value="3" name="rating">\n\t\t\t\t\t\t\t\t\t\t\t\t<input type="radio" class="rating__item" value="4" name="rating">\n\t\t\t\t\t\t\t\t\t\t\t\t<input type="radio" class="rating__item" value="5" name="rating">\n\t\t\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t\t<div class="rating__value">${item.starRatingValue}</div>\n\t\t\t\t\t\t\t\t\t\t<div class="rating__quantity">(${item.starRatingQuantity})</div>\n\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t</div>\n  `;
            if (item.priceName && item.priceValue && item.button) trailersItemTemplate += `\n  <div class="item-products__bottom bottom-item-products">\n\t\t\t\t\t\t\t\t\t<div class="bottom-item-products__price price-bottom-item-products">\n\t\t\t\t\t\t\t\t\t\t<div class="price-bottom-item-products__name">${item.priceName}</div>\n\t\t\t\t\t\t\t\t\t\t<div class="price-bottom-item-products__value">${item.priceValue}</div>\n\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t<button type="button" class="bottom-item-products__button button button_gray">${item.button}</button>\n\t\t\t\t\t\t\t\t</div>\n  `;
            trailersItemTemplate += `</article>`;
            return trailersItemTemplate;
        }
        function updateViewMoreButton(blockKey) {
            const blockData = data[blockKey];
            const containerSelector = blocksConfig[blockKey].containerSelector;
            const container = document.querySelector(containerSelector);
            const viewMoreButton = document.querySelector(blocksConfig[blockKey].buttonSelector);
            if (blockData && container && viewMoreButton) {
                const dataItems = blockData.items;
                const dataItemsLength = dataItems ? dataItems.length : 0;
                const currentItems = container.querySelectorAll(`.${containerSelector.replace(".", "")} > *`).length;
                if (currentItems < dataItemsLength) viewMoreButton.classList.remove("_hidden"); else viewMoreButton.classList.add("_hidden");
            }
        }
        function initBlock(blockKey, config) {
            const {containerSelector, buttonSelector, loadMoreCount} = config;
            const container = document.querySelector(containerSelector);
            const viewMoreButton = document.querySelector(buttonSelector);
            if (container && viewMoreButton) {
                viewMoreButton.addEventListener("click", (function() {
                    startItem[blockKey] = endItem[blockKey];
                    endItem[blockKey] += loadMoreCount;
                    loadBlockData(blockKey);
                }));
                loadBlockData(blockKey);
            }
        }
        async function loadBlockData(blockKey) {
            const container = document.querySelector(blocksConfig[blockKey].containerSelector);
            const viewMoreButton = document.querySelector(blocksConfig[blockKey].buttonSelector);
            if (container && viewMoreButton) try {
                if (!data[blockKey]) {
                    const response = await fetch("files/server.json", {
                        method: "GET"
                    });
                    if (response.ok) {
                        const responseResult = await response.json();
                        data[blockKey] = responseResult.blocks[blockKey];
                    } else throw new Error(`Ошибка при выборке данных для блока ${blockKey}`);
                }
                const blockData = data[blockKey];
                const dataPart = blockData.items.slice(startItem[blockKey], endItem[blockKey]);
                dataPart.forEach((item => {
                    buildItem(container, item, blockKey);
                }));
                updateViewMoreButton(blockKey);
                if (!blockData.hasMore) viewMoreButton.classList.add("_hidden");
            } catch (error) {
                console.error(error);
